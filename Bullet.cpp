...
void Bullet_Attack1::update_image(){
	if (!deleting){
		...
		else if((posx == array[3]) && (posy <= array[4]-1)){	//Case 2: not in the last line, last element of row
			posx = 0;
			posy ++;
		}
		else if((posx < array[5]) && (posy == array[4])){	//Case 3: last line, not last element of row
			posx ++;
		}
		else if((posx == array[5]) && (posy == array[4])){	//Case 4: last line, last element of row
			posx = 0;
			posy = 0;
		}
		bullet.setTexture(bpic);
		bullet.setTextureRect(sf::IntRect(posx*(array[2]), posy*(array[2]), array[2], array[2]));
	}
	else{
		if((posx < array[10]) && (posy <= array[11]-1)){		//Case 1:  not in the last line, not last element of row
			posx++;
		}
		else if((posx == array[10]) && (posy <= array[11]-1)){	//Case 2: not in the last line, last element of row
			posx = 0;
			posy ++;
		}
		else if((posx < array[12]) && (posy == array[11])){		//Case 3: last line, not last element of row
			posx ++;
		}
		else if((posx == array[12]) && (posy == array[11])){	//Case 4: last line, last element of row
			deleted = true;
		}

		bullet.setTexture(deleting_pic);
		bullet.setTextureRect(sf::IntRect(array[7]+posx*(array[9]),array[8]+posy*(array[9]),array[9],array[9]));
		bullet.setScale(sf::Vector2f(array[13], array[13]));
	}
}
...
void Bullet_Attack1::deletion(){
	deleting = true;
	bullet.setTexture(deleting_pic);
	bullet.setTextureRect(sf::IntRect(array[7]*array[9],array[8]*array[9],array[9],array[9]));
	bullet.setScale(sf::Vector2f(array[13], array[13]));
	posx = 0;
	posy = 0;
}