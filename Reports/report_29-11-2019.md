...
Anya finished the health bar class, which shows the level of health of a pokemon during a fight and changes size and color as the pokemon loses health. She sorted the code into further classes, creating the Artillery class so that the handling of the bullets is no longer controlled in the Pokemon class. She started working on the Opponent's shooting, allowing it to shoot with a 15% probability, to the correct position. Next week, she will improve the shooting of the opponent, adding slight error in how it shoots,etc... (make it more human). 
